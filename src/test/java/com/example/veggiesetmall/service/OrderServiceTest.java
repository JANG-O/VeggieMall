...
import com.example.veggiesetmall.domain.item.Book;
...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        ...
        assertEquals("상품 주문시 상태는 ORDER", OrderStatus.ORDER, getOrder.getStatus()); // Assert.assertEquals()
        assertEquals("주문한 상품 종류 수가 저확해야 한다.", 1, getOrder.getOrderItems().size());
        assertEquals("주문 가격은 가격 * 수량이다.", 10000 * orderCount, getOrder.getTotalPrice());
        assertEquals("주문 수량만큼 재고가 줄어야 한다.", 12, book.getStockQuantity());  // 8 --> 12 ???
    }
    ...
    public void 주문취소() throws Exception {
        ...
        assertEquals("주문이 취소된 상품은 그만큼 재고가 증가해야 한다.", 14, item.getStockQuantity());  // 10 --> 14 ???
    }
